...
use crate::{
    ...
    components::linkhub::seeker::{
        calculate_base_dealing, change_resource_dealing, get_resource_description, get_resource_status_str, send_intent
    }, 
    tools::llmq::prompt
};
...
pub async fn reroute(s_intent: &mut SubIntent)  -> BoxResult<()> {
    ...
    {
        change_resource_dealing(name, true).await;
...
async fn score(sub_intent: &str, resource: &str) -> u64 {
    match SCORE_METHOD {
        ...
        "usage" => {
            ...
            u64::MAX - calculate_base_dealing(resource).await
        },
        _ => {
