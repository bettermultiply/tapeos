        ...
        tokio::select! {
            ...
            Ok((amt, src)) = socket.recv_from(&mut buf) => {
                ...
                tokio::spawn(async move{
                    let _ = message_handler(src, amt, c_tape_i, c_tape_o, &buf, c_socket, c_m_json, c_status, c_time, c_times).await;
                });
...
async fn message_handler(
    ...
    usage_time:  Arc<Mutex<u64>>,
    usage_times:  Arc<Mutex<u64>>,
) -> BoxResult<()> {
    ...
    match m.get_type() {
        ...
        MessageType::Finish => {
            ...
            *tape_o.lock().await = None;
            println!("{};{}", usage_times.lock().await, usage_time.lock().await);
            return Ok(());
        }
